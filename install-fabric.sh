...
cloneSamplesRepo() {
    ...
    if [ -d test-network ]; then
        ...
        echo "===> Cloning agxpro-z/property-record repo"
        git clone -b main https://github.com/agxpro-z/property-record && cd property-record
    fi

    if GIT_DIR=.git git rev-parse v${VERSION} >/dev/null 2>&1; then
        echo "===> Checking out v${VERSION} of agxpro-z/property-record"
...
download() {
    ...
    # if [ -d property-record ]; then
    #    DEST_DIR="property-record"
    # fi
